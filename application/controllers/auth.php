...
class Auth extends CI_Controller
{
	public function login()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($cek == FALSE) {
				$this->session->set_flashdata('pesan_password_salah', 'Username atau Password Salah!');
				redirect('auth/login');
			} else {
	...
	public function ganti_password_aksi()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->session->set_flashdata('pesan_password_sukses', 'Password Berhasil di Update,Silahkan Login!');
			redirect('auth/login');
		}
	}
}
